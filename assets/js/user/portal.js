...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const participantNumberElement =
  ...
  await fetch("/api/get_user_data")
    ...
    .then((data) => {
      ...
      function drawStaminaWheel(value) {
        ...
        let data = {
          labels: ["Your Points", "Remaining Points"],
          datasets: [
            {
              label: "Points",
      ...
      $("#calendar").fullCalendar({
        ...
        events: data.selectedDays
          ...
          .concat(
            data.completedPlannedActivities?.map((activity) => {
              return {
          ...
          .concat(
            missedPlannedActivities?.map((activity) => {
              return {
      ...
      let firstDate =
        weeklyPointsData.data[weeklyPointsData.data.length - 1].date;
