  ...
  if ((newEmail && newEmail.length > 0 && newEmail.includes("@")) || newPhone) {
    try {
      const response = await fetch("/admin/submit-contact", {
        ...
        body: JSON.stringify({
          identifier: newEmail || newPhone,
          identifier_type: newEmail ? "email" : "phone",
...
async function deleteHealthContact() {
  const selectedContact =
    document.getElementById("removeHealthEmail").value ||
    document.getElementById("removeHealthPhone").value;
...
document.getElementById("points").addEventListener("click", async function () {
  ...
  pointsResult.textContent = `Participant ${selectedOption.getAttribute(
...
document
  .getElementById("addHealthContact")
  .addEventListener("click", function (event) {
    event.preventDefault(); // Prevent the default form submission behavior
...
document
  .getElementById("removeHealthContact")
  .addEventListener("click", function (event) {
    event.preventDefault(); // Prevent the default form submission behavior
